...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-[20.5rem] flex-col overflow-y-hidden bg-basecolor duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
              ...
              {user ? (
                <div>
                  <li className="mb-[0.85rem]">
                    <NavLink
                      to="/"
                      className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-black dark:hover:bg-meta-4 ${
                        (pathname.endsWith('/') ||
